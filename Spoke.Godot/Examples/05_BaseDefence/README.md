...
A complete RTS-lite / Tower Defence game, ported from the Unity example of the same name.

It was built to show how a game looks when it goes all-in on Spoke's idioms, using `SpokeNode2D`, `SpokeNode` and `SpokeControl` in place of `Node2D`, `Node` and `Control` throughout.
...
Unity's version is 3D and measured in metres; this one is 2D and measured in pixels. Every exported value stays in the original's metres, and `World.PixelsPerMetre` is the only place the two meet.
